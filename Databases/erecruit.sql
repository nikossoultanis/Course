...
personality int(5),			/*for the score */
PRIMARY KEY (username),
...
jobid INT(4),
interv_start DATETIME NOT NULL,
duration DATETIME NOT NULL,
comments VARCHAR(50) DEFAULT NULL,
score SMALLINT NOT NULL,
PRIMARY KEY (recruiter,candidate,jobid)
CONSTRAINT RECR FOREIGN KEY (recruiter) REFERENCES recruiter(username) ON DELETE CASCADE ON UPDATE CASCADE
CONSTRAINT CAND FOREIGN KEY (candidate) REFERENCES candidate(username) ON DELETE CASCADE ON UPDATE CASCADE
CONSTRAINT JOB  FOREIGN KEY (jobid)       REFERENCES job(id)             ON DELETE CASCADE ON UPDATE CASCADE

CREATE TABLE sector (
  title VARCHAR(50) NOT NULL,
  etaireia char(9) NOT NULL,
  descr VARCHAR(50) DEFAULT NULL,
  supersec VARCHAR(50) DEFAULT NULL,
  PRIMARY KEY (title,etaireia),
  CONSTRAINT ETAIR FOREIGN KEY (etaireia) REFERENCES etaireia(AFM) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT SUPER FOREIGN KEY (supersec) REFERENCES sector(title) ON DELETE CASCADE ON UPDATE CASCADE
)engine=InnoDB;
